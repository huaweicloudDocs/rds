...
-   接口约束
    ...
    -   目前支持查询的异步任务：三款引擎\(MySQL, PostgreSQL, Microsoft SQL\)的创建实例\(单实例/主备实例\)、创建只读实例\(Microsoft SQL不支持创建只读实例\)、单实例转主备实例、主备倒换、重启实例、调整实例容量、绑定和解绑弹性IP、MySQL的开启和关闭远程连接、表级时间点恢复。
