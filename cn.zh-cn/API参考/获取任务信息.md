...
-   目前支持查询的主要异步任务：创建实例\(单实例/主备实例\)、创建只读实例、单实例转主备实例、主备切换、磁盘扩容、绑定和解绑弹性IP，恢复到新实例，MySQL实例迁移可用区，MySQL实例小版本升级。
...
-   正常响应要素说明
    ...
    **表 4**  entities字段数据结构说明\(创建实例，单转主备，创建只读实例\)
